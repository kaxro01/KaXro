...
const COMPONENT_TIMEOUT = 60 * 1000;
...
function getColor(hex) {
    return isValidHex(hex) ? parseInt(hex.slice(1), 16) : 0x5865F2;
}
...
function buildPreview(state) {
    const embed = new EmbedBuilder().setColor(getColor(state.color));
    ...
    if (state.author?.name) {
        const author = { name: state.author.name.slice(0, 256) };
    ...
    if (state.footer?.text) {
        const footer = { text: state.footer.text.slice(0, 2048) };
    ...
    if (
        ...
        embed.setDescription('*(Empty embed — use the buttons below to add content.)*');
    }
...
function buildDashboard(state) {
    ...
    return new EmbedBuilder()
        ...
        .setFooter({ text: 'KaXro Embed Builder • Session expires after 5 minutes' });
}
...
// Discord rejects some modal text inputs when an empty value is explicitly sent.
// This helper only calls setValue when there is an actual value to pre-fill.
function optionalValue(input, value) {
    if (value) input.setValue(value);
    return input;
}

function buildContentModal(state) {
    const title = optionalValue(
...
function buildFieldModal(state, index = null) {
    const field = index !== null ? state.fields[index] : null;
    ...
    const inline = optionalValue(
        new TextInputBuilder()
            ...
            .setRequired(false)
            .setPlaceholder('no'),
        field ? (field.inline ? 'yes' : 'no') : 'no',
    );

    return new ModalBuilder()
        .setCustomId(index === null ? 'eb_modal_add_field' : `eb_modal_edit_field_${index}`)
        .setTitle(index === null ? 'Add Field' : `Edit Field #${index + 1}`)
        .addComponents(
            new ActionRowBuilder().addComponents(name),
            ...
            .addOptions(
                state.fields.map((field, index) =>
                    new StringSelectMenuOptionBuilder()
                        .setLabel(`${index + 1}. ${field.name}`.slice(0, 100))
...
async function refresh(interaction, state) {
    await interaction.editReply({
        embeds: [buildPreview(state), buildDashboard(state)],
...
async function errorReply(interaction, message) {
    ...
    try {
        ...
        // The interaction may already have expired or been acknowledged.
    }
}

async function waitForEphemeralComponent(interaction, payload, customId) {
    // Ephemeral replies do not exist in the normal channel message stream.
    // Fetch the ephemeral reply itself and collect the component from that message.
...
async function handleContent(interaction, state) {
    ...
    state.title = submitted.fields.getTextInputValue('title').trim();
    state.description = submitted.fields.getTextInputValue('description').trim();

    await submitted.deferUpdate();
}

async function handleAuthor(interaction, state) {
    await interaction.showModal(buildAuthorModal(state));
    ...
    if (icon && !isValidUrl(icon)) {
        await errorReply(submitted, 'The author icon must be a valid HTTP/HTTPS URL.');
        return;
    }

    if (url && !isValidUrl(url)) {
        await errorReply(submitted, 'The author URL must be a valid HTTP/HTTPS URL.');
        return;
    }

    state.author = name ? { name, iconUrl: icon || null, url: url || null } : null;
    await submitted.deferUpdate();
}

async function handleFooter(interaction, state) {
    await interaction.showModal(buildFooterModal(state));
    ...
    if (icon && !isValidUrl(icon)) {
        await errorReply(submitted, 'The footer icon must be a valid HTTP/HTTPS URL.');
        return;
    }

    state.footer = text ? { text, iconUrl: icon || null } : null;
    await submitted.deferUpdate();
}

async function handleAddField(interaction, state) {
    if (state.fields.length >= MAX_FIELDS) {
        await errorReply(interaction, `You can only have ${MAX_FIELDS} fields.`);
        return;
    }
    ...
    const inline = submitted.fields.getTextInputValue('inline').trim().toLowerCase() === 'yes';
...
async function handleEditField(interaction, state) {
    ...
    const select = await waitForEphemeralComponent(
        interaction,
        {
            content: '📝 Select the field you want to edit:',
            components: [buildFieldSelect(state, 'eb_edit_field_select')],
        },
        'eb_edit_field_select',
    );
    ...
    await select.showModal(buildFieldModal(state, index));
    ...
    state.fields[index] = { name, value, inline };
    await submitted.deferUpdate();
}
...
async function handleRemoveField(interaction, state) {
    ...
    const select = await waitForEphemeralComponent(
        interaction,
        {
            ...
            components: [buildFieldSelect(state, 'eb_remove_field_select')],
        },
        'eb_remove_field_select',
    );
...
async function handleColor(interaction, state) {
    ...
    if (selected === 'custom') {
        ...
        const hex = submitted.fields.getTextInputValue('hex').trim();

        if (!isValidHex(hex)) {
            await errorReply(submitted, 'Invalid color. Use the format `#RRGGBB`.');
            return;
        }
...
async function handleImages(interaction, state) {
    ...
    if (choice === 'clear_thumbnail') {
        ...
        await select.update({ content: '✅ Thumbnail removed.', components: [] });
        return;
    }

    if (choice === 'clear_image') {
        state.image = null;
        await select.update({ content: '✅ Large image removed.', components: [] });
        return;
    }

    const modal = new ModalBuilder()
        .setCustomId(`eb_modal_${choice}`)
        .setTitle(choice === 'thumbnail' ? 'Set Thumbnail' : 'Set Large Image')
        .addComponents(
            new ActionRowBuilder().addComponents(
                new TextInputBuilder()
                    .setCustomId('url')
    ...
    if (!isValidUrl(url)) {
        await errorReply(submitted, 'Please provide a valid HTTP/HTTPS image URL.');
        return;
    }

    if (choice === 'thumbnail') state.thumbnail = url;
...
async function handleExport(interaction, state) {
    ...
    if (raw.length <= 1900) {
        ...
        await interaction.reply({
            content: '📋 The JSON data is too large to display in Discord.',
            flags: MessageFlags.Ephemeral,
        });
    }
}
...
async function handlePost(interaction, state) {
    ...
    await interaction.channel.send({ embeds: [buildPreview(state)] });
}
    ...
    async execute(interaction) {
        ...
        collector.on('collect', async component => {
            try {
                ...
                await errorReply(component, 'Something went wrong while using the Embed Builder.');
            }
        });
        ...
        collector.on('end', async () => {
            try {
                ...
                // Original interaction message is no longer available.
            }
        });
    },
};
